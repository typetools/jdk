  ...
  - job: build_jdk25u
    ...
    steps:
      ...
      - bash: |
          set -ex
          echo "pwd = $(pwd)"
          if test -d ../jdk25u; then
            echo "../jdk25u should not exist yet"
            false
          fi
          df .
          ...
          cd ../jdk25u
          git diff --exit-code
        displayName: clone-related-jdk25u
      - bash: |
          git config --global --add safe.directory $(cd ../jdk25u && pwd)
          cd ../jdk25u && git status
      ...
      - bash: |
          ...
          set
          echo "About to run: git pull --no-edit https://github.com/${CI_ORGANIZATION}/jdk ${CI_BRANCH}"
          cd ../jdk25u && git pull --no-edit https://github.com/${CI_ORGANIZATION}/jdk ${CI_BRANCH} || (git --version && git show | head -100 && git status && git diff | head -1000 && echo "Merge failed; see 'Pull request merge conflicts' at https://github.com/typetools/jdk/blob/master/README.md " && false)
        displayName: git merge
      - bash: cd ../jdk25u && export JT_HOME=/usr/share/jtreg && bash ./configure --with-jtreg --disable-warnings-as-errors
        displayName: configure
      - bash: cd ../jdk25u && make jdk
        displayName: make jdk
        ## This works only after `make images`
        # - bash: cd ../jdk25u && build/*/images/jdk/bin/java -version
        #   displayName: version
        # - bash: make -C /jdk25u run-test-tier1
        #   timeoutInMinutes: 0
        #   displayName: make run-test-tier1
        # - bash: make -C /jdk25u :test/jdk:tier1
        ## Temporarily comment out because of trouble finding junit and jasm
        # - bash: cd ../jdk25u && make run-test TEST="jtreg:test/jdk:tier1"
        #   timeoutInMinutes: 0
